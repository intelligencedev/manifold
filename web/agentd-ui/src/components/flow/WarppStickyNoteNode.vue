<template>
  <div class="sticky-note-node relative h-full w-full select-none">
    ...
    <div class="sticky-header" :class="{ 'pointer-events-none': !isDesignMode }">
      ...
      <div v-if="isDesignMode" class="flex items-center gap-1 ml-auto">
        ...
        <div v-if="showColorPicker" class="color-picker-popup">
          ...
          >
            <svg v-if="noteColor === preset.value" class="h-3 w-3 text-white drop-shadow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
...
<style scoped>
.sticky-note-node { border-radius: 10px; }
.sticky-surface {
  position: absolute;
  inset: 0;
  border-radius: 10px;
  background: color-mix(in srgb, rgb(var(--color-surface) / 1) 80%, transparent 20%);
...
.color-swatch:hover { border-color: rgb(var(--color-foreground) / 0.6); transform: scale(1.1); }
.color-swatch.active { border-color: rgb(var(--color-accent)); box-shadow: 0 0 0 2px rgb(var(--color-accent) / 0.3); }
.sticky-body { position: absolute; top: 2rem; left: 0.5rem; right: 0.5rem; bottom: 0.5rem; }
.sticky-textarea {
