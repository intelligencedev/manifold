<template>
  <div :class="['fixed top-[64px] bottom-0 w-[272px] z-[1100] transition-transform duration-300 text-white bg-zinc-900 flex flex-col space-y-2 shadow-[5px_0_10px_-5px_rgba(0,0,0,0.3)] tracking-wider', isOpen ? 'translate-x-0' : '-translate-x-full']">
    ...
    <div class="p-4 h-full box-border">
      <div class="scrollbar-container overflow-y-auto h-full pr-2 space-y-4 palette-scroll">
        <div v-for="(nodes, category) in nodeCategories" :key="category" class="content-container max-w-[240px]">
...
/* Make scrollbars overlay content rather than take up space */
.scrollbar-container {
  overflow-y: overlay; /* This is non-standard but works in modern browsers */
  scrollbar-gutter: stable; /* CSS standard way to reserve gutter */
}

/* Ensure elements have a fixed width that accounts for scrollbar */
.content-container {
  width: 100%;
