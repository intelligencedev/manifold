...
func (a *app) executeFlowV2Run(ctx context.Context, userID int64, runID string, wf flow.Workflow, plan *flow.Plan, input map[string]any) {
	emit := func(ev flow.RunEvent) {
		if durableEvent, err := durable.RecordEvent(ctx, "flow."+string(ev.Type), flowEventPayload(ev)); err == nil {
	...
	launchNode := func(nodeID string) bool {
		...
		go func(node flow.Node) {
			if checkpointed, found, err := loadDurableNodeCheckpoint(runCtx, node.ID); err != nil {
				resultCh <- flowNodeResult{nodeID: node.ID, err: err}
				return
			} else if found {
				resultCh <- flowNodeResult{nodeID: node.ID, output: checkpointed}
				return
			}
			...
			output, err := durable.Step[map[string]any](runCtx, "node:"+node.ID, func(stepCtx context.Context) (map[string]any, error) {
				return a.executeFlowV2NodeWithRetries(stepCtx, node, resolvedInputs, reg, toolSet, defaultExec, emit)
			})
