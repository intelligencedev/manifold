...
- **Supporting subsystems:**
  ...
  - `internal/tools` holds the dynamic registry, tool schemas, and concrete tool packages (CLI, web, RAG, patch, utility, specialists, multi-tool wrapper, etc.).
...
- **Data/control flow (simplified):**
  ...
  graph TD
    ...
    TOOLS --> MCP
  ```
  The binaries differ mainly in how they source input (CLI flags or HTTP requests) and how results are exposed (stdout or HTTP responses). Internally they reuse the same modules.

## 3. Modules and packages
- **./cmd/**
  - Houses the Go entrypoints (`agent`, `agentd`, `whisper-go`, experimental `adk`). Each subdirectory has a `main.go` that wires config + internal packages for that binary.
...
- **./internal/warpp**
  - Workflow definitions: loader, guards, runner, personalization, tool gating, and StepPublisher wiring. Used by the agent CLI (WARPP mode) and agentd.

- **./internal/tools**
- Registry plus concrete tool implementations under subpackages (`cli`, `web`, `patchtool`, `rag`, `llmtool`, `specialists`, `imagetool`, `tts`, `utility`, `textsplitter`, `warpptool`, etc.). Tools expose JSON schemas, get registered per-process, and are dispatched via the engine.
...
- **./internal/projects**
  - Filesystem-backed project service. Serves `/api/projects` endpoints for storing per-user artifacts on disk.
...
- **./internal/config**
  - Config structs + loader. Defines everything the binaries rely on: LLM client selection, tool toggles, MCP servers, TTS, database DSNs, auth, etc.
...
- **./cmd/agent/main.go**
  ...
  2. Optionally hydrate specialists from Postgres via `databases.NewSpecialistsStore`, falling back to YAML. When a specialist exists it can override OpenAI settings.
...
- **./cmd/agentd/main.go → internal/agentd.Run()**
  ...
  2. Build a base tool registry (CLI, web search/fetch/screenshot, patch, textsplitter, textbox, TTS, RAG ingest/retrieve, llm_transform, image describe, specialists, agent-call, ask_agent) and filter it per `enableTools` and allow lists.
...
6. **`internal/warpp` + `docs/warpp.md` + `examples/workflows/*`** – to extend deterministic workflows, read how intents, guards, and tool references are structured.
7. **`internal/persistence/databases/factory.go` & RAG/playground packages** – explore how data storage backends are swapped and how higher-level services (playground experiments, RAG ingestion) piggyback on the same manager.
