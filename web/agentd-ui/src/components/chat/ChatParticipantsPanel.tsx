...
function renderParticipantActivityGroup(
  ...
  return (
    <div class="participant-activity-card" key={model.participantActivityGroupKey(group)}>
      <button
        type="button"
        class={[
        ...
        <div class="participant-activity-detail">
          <div class="participant-activity-detail-item">
            <div class="participant-activity-detail-header">
              <span class="participant-activity-detail-title">{thread.name}</span>
            ...
            <div
              ...
              onScroll={(event) => model.handleThreadBodyScroll(event, thread.id)}
            >
              ...
              {thread.error ? <p class="activity-error-text">{thread.error}</p> : null}
            </div>
            {group.children.length ? (
              <div class="participant-activity-children">
                {group.children.map((child) => renderParticipantActivityGroup(child, model))}
              </div>
            ) : null}
          </div>
        </div>
...
function renderParticipant(participant: Participant, model: ChatParticipantsPanelModel) {
  ...
  return (
    <li key={participant.id} class="participant-list-item">
      ...
      >
        <span class={["participant-dot", model.participantDotClasses(participant)]} />
        ...
        <span class="participant-status">{model.participantStatusLabel(participant)}</span>
      </button>
      {activityGroups.length ? (
        <div class="participant-activity-list">
          {activityGroups.map((group) => renderParticipantActivityGroup(group, model))}
        </div>
      ) : null}
    </li>
  );
