  ...
  <div class="flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div class="sticky top-0 z-10 border-b border-border/50 bg-surface/90 backdrop-blur-sm">
      <div class="flex items-start justify-between gap-3 px-4 pb-3 pt-4">
        <div class="min-w-0">
          <h2 class="text-base font-semibold text-foreground">{{ headerTitle }}</h2>
          <p v-if="headerSubtitle" class="mt-0.5 text-xs text-subtle-foreground">{{ headerSubtitle }}</p>
        </div>
        <div class="flex items-center gap-2">
          <span v-if="isDirty" class="rounded-full border border-border/60 bg-surface-muted/30 px-2 py-1 text-xs font-semibold text-subtle-foreground">Unsaved</span>
          <button
            type="button"
      ...
      <div role="tablist" aria-label="Edit Specialist" class="flex flex-wrap gap-2 px-4 pb-3">
        <button
          v-for="t in tabs"
          :key="t.id"
          role="tab"
          ...
          :class="activeTab === t.id ? 'border-border/80 bg-surface-muted/60 text-foreground' : 'border-border/50 text-subtle-foreground hover:border-border'"
          @click="activeTab = t.id"
          @keydown="onTabKeydown($event, t.id)"
        >
          <span>{{ t.label }}</span>
          <span v-if="tabHasErrors(t.id)" class="ml-2 inline-flex h-1.5 w-1.5 rounded-full bg-danger"></span>
        </button>
      </div>
    </div>
    ...
    <div class="flex flex-1 min-h-0 flex-col overflow-auto px-4 pb-6 pt-4 scrollbar-inset">
      <div v-if="actionError" class="mb-4 rounded-2xl border border-danger/60 bg-danger/10 p-3 text-danger-foreground text-sm">
        {{ actionError }}
      </div>
      <div v-if="successMsg" class="mb-4 rounded-2xl border border-border/60 bg-surface-muted/30 p-3 text-sm text-foreground">
        {{ successMsg }}
      </div>
      ...
      >
        <div v-if="submitAttempted && errorsByTab.basics.length" class="rounded-2xl border border-danger/60 bg-danger/10 p-3 text-sm text-danger-foreground">
        ...
        <FormSection title="Identity" helper="Give the specialist a stable name and a short description.">
          <div class="flex flex-col gap-3">
            <div class="flex flex-col gap-1">
              <label for="sp-name" class="text-xs font-semibold uppercase tracking-wide text-subtle-foreground">Name</label>
              <input
                id="sp-name"
                ...
                @blur="touch('name')"
              />
              <p v-if="fieldError('name')" class="text-xs text-danger-foreground">{{ fieldError('name') }}</p>
            </div>

            <div class="flex flex-col gap-1">
              <label for="sp-description" class="text-xs font-semibold uppercase tracking-wide text-subtle-foreground">Description</label>
              <textarea
                id="sp-description"
        ...
        <FormSection title="Status" helper="Paused specialists are not available for use and do not consume orchestration context.">
          <div class="flex flex-col gap-3">
            <label class="inline-flex items-center justify-between gap-3 rounded border border-border/60 bg-surface-muted/20 px-3 py-2">
              ...
              <input
                ...
                @change="draft.paused = !(($event.target as HTMLInputElement).checked)"
              />
            </label>
            <label class="inline-flex items-center justify-between gap-3 rounded border border-border/60 bg-surface-muted/20 px-3 py-2">
              <span class="text-sm text-foreground">Paused</span>
              <input id="sp-paused" v-model="draft.paused" type="checkbox" class="h-4 w-4" />
            </label>
          </div>
        </FormSection>

        <FormSection title="Runtime" helper="Select a provider and model. Optionally override the default endpoint.">
          <div class="flex flex-col gap-3">
            <div class="grid gap-3 md:grid-cols-2">
              <div class="flex flex-col gap-1">
                <label for="sp-provider" class="text-xs font-semibold uppercase tracking-wide text-subtle-foreground">Provider</label>
                <DropdownSelect
                  id="sp-provider"
                ...
                />
                <p v-if="fieldError('provider')" class="text-xs text-danger-foreground">{{ fieldError('provider') }}</p>
              </div>
              <div class="flex flex-col gap-1">
                <label for="sp-model" class="text-xs font-semibold uppercase tracking-wide text-subtle-foreground">Model</label>
                <input
                  id="sp-model"
                  ...
                  @blur="touch('model')"
                />
                <p v-if="fieldError('model')" class="text-xs text-danger-foreground">{{ fieldError('model') }}</p>
              </div>
            </div>

            <div class="rounded border border-border/60 bg-surface-muted/20 p-3">
              <label class="inline-flex items-center gap-2 text-sm text-foreground">
                <input v-model="draft.useDefaultEndpoint" type="checkbox" class="h-4 w-4" />
                <span>Use default endpoint (recommended)</span>
              </label>
              <p class="mt-1 text-xs text-subtle-foreground">Stores only overrides when you provide a custom endpoint.</p>

              <div v-if="!draft.useDefaultEndpoint" class="mt-3 flex flex-col gap-1">
                <label for="sp-baseurl" class="text-xs font-semibold uppercase tracking-wide text-subtle-foreground">Custom endpoint</label>
                <input
                  id="sp-baseurl"
                  ...
                  @blur="touch('baseURL')"
                />
                <p v-if="fieldError('baseURL')" class="text-xs text-danger-foreground">{{ fieldError('baseURL') }}</p>
              </div>
              <div v-else class="mt-3 text-xs text-subtle-foreground">
                Default: <span class="font-mono">{{ defaultBaseURL || '—' }}</span>
              </div>
            </div>
          </div>
        </FormSection>

        <FormSection title="Credentials" helper="Secrets are never re-displayed after save.">
          <div class="flex flex-col gap-3">
            <div class="flex items-center justify-between gap-3 rounded border border-border/60 bg-surface-muted/20 px-3 py-2">
              <div>
                <p class="text-sm font-medium text-foreground">API key</p>
                <p class="text-xs text-subtle-foreground">{{ credentialStatus }}</p>
              </div>
              <button
                type="button"
      ...
      >
        <div v-if="submitAttempted && errorsByTab.prompt.length" class="rounded-2xl border border-danger/60 bg-danger/10 p-3 text-sm text-danger-foreground">
          <p class="font-semibold">Fix the following to continue:</p>
          <ul class="mt-2 list-disc pl-5">
        ...
        <FormSection title="Prompt" helper="Edit the system prompt. Optionally apply a saved template/version." class="flex min-h-0 flex-1 flex-col">
          <div class="flex min-h-0 flex-1 flex-col gap-3">
            <div class="grid gap-3 md:grid-cols-2">
              <div class="flex flex-col gap-1">
                <label for="prompt-select" class="text-xs font-semibold uppercase tracking-wide text-subtle-foreground">Template</label>
                <DropdownSelect
                  id="prompt-select"
            ...
            <div class="flex items-center justify-between gap-2">
              <p v-if="applyVersionError" class="text-xs text-danger-foreground">{{ applyVersionError }}</p>
              <div class="flex items-center gap-2">
                <button
                  type="button"
            ...
            <div class="flex min-h-0 flex-1 flex-col">
              <label for="sp-system" class="text-xs font-semibold uppercase tracking-wide text-subtle-foreground">System prompt</label>
              <div class="mt-2 flex min-h-0 flex-1 flex-col">
                ...
                >
                  <template #left>
                    <span>Used as the system instruction for this specialist.</span>
                  </template>
                </CodeEditor>
              </div>
              <p v-if="fieldError('system')" class="mt-1 text-xs text-danger-foreground">{{ fieldError('system') }}</p>
            </div>
          </div>
        </FormSection>
      </div>
      ...
      <div
        ...
        class="flex flex-col gap-4"
      >
        <div v-if="submitAttempted && errorsByTab.tools.length" class="rounded-2xl border border-danger/60 bg-danger/10 p-3 text-sm text-danger-foreground">
          <p class="font-semibold">Fix the following to continue:</p>
          <ul class="mt-2 list-disc pl-5">
        ...
        <FormSection title="Tool access policy" helper="Choose whether this specialist may call tools.">
          <div class="grid grid-cols-3 gap-2">
            <label
              class="flex cursor-pointer items-start gap-3 rounded-lg border px-3 py-3 transition-colors"
              :class="draft.toolPolicy === 'none' ? 'border-border/80 bg-surface-muted/60' : 'border-border/50 hover:border-border'"
            >
              <input class="mt-1 h-4 w-4 shrink-0" type="radio" name="tools-policy" value="none" v-model="draft.toolPolicy" />
              <div>
            ...
            <label
              class="flex cursor-pointer items-start gap-3 rounded-lg border px-3 py-3 transition-colors"
              :class="draft.toolPolicy === 'any' ? 'border-border/80 bg-surface-muted/60' : 'border-border/50 hover:border-border'"
            >
              <input class="mt-1 h-4 w-4 shrink-0" type="radio" name="tools-policy" value="any" v-model="draft.toolPolicy" />
              <div>
            ...
            <label
              class="flex cursor-pointer items-start gap-3 rounded-lg border px-3 py-3 transition-colors"
              :class="draft.toolPolicy === 'allow-list' ? 'border-border/80 bg-surface-muted/60' : 'border-border/50 hover:border-border'"
            >
              <input class="mt-1 h-4 w-4 shrink-0" type="radio" name="tools-policy" value="allow-list" v-model="draft.toolPolicy" />
              <div>
        ...
        >
          <div class="flex flex-col gap-3">
            <div class="flex items-center justify-between gap-3">
              <p class="text-sm text-muted-foreground">Selected: <span class="font-semibold text-foreground">{{ allowTools.length }}</span></p>
            </div>

            <div>
              <label for="sp-tools-search" class="text-xs font-semibold uppercase tracking-wide text-subtle-foreground">Search tools</label>
              <input
                id="sp-tools-search"
                v-model="toolsSearch"
                type="text"
            ...
            <p v-if="toolsLoading" class="text-xs text-subtle-foreground">Loading tools…</p>
            <p v-else-if="toolsError" class="text-xs text-danger-foreground">{{ toolsError }}</p>

            <div v-else class="rounded-lg border border-border/60 bg-surface">
              <div v-if="!filteredTools.length" class="px-3 py-3 text-sm text-subtle-foreground">No tools match your search.</div>
              <label
              ...
              >
                <input
                  ...
                  @change="setToolAllowed(t.name, ($event.target as HTMLInputElement).checked)"
                />
                <div class="min-w-0">
                  <p class="text-sm font-medium text-foreground">{{ t.name }}</p>
                  <p v-if="t.description" class="mt-0.5 text-xs text-subtle-foreground">
                    {{ t.description }}
                  </p>
                </div>
              </label>
      ...
      <div
        ...
        class="flex flex-col gap-4"
      >
        <div v-if="submitAttempted && errorsByTab.advanced.length" class="rounded-2xl border border-danger/60 bg-danger/10 p-3 text-sm text-danger-foreground">
          <p class="font-semibold">Fix the following to continue:</p>
          <ul class="mt-2 list-disc pl-5">
        ...
        <CollapsiblePanel v-model="headersOpen" title="Extra headers" helper="Sent with requests made by this specialist.">
          <KeyValueTableEditor
        ...
        <CollapsiblePanel v-model="paramsOpen" title="Extra params" helper="Additional provider parameters.">
          <KeyValueTableEditor
    ...
    <div class="sticky bottom-0 z-10 border-t border-border/50 bg-surface/90 backdrop-blur-sm">
      <div class="flex items-center justify-between gap-3 px-4 py-3">
        <div class="text-xs text-subtle-foreground">
          <span v-if="isDirty">Changes not saved.</span>
    ...
    <!-- Credential modal -->
    <div v-if="showCredentialModal" class="fixed inset-0 z-50 flex items-center justify-center px-4 py-8" @keydown="onCredentialKeydown">
      <div class="absolute inset-0 bg-surface/70 backdrop-blur-sm" @click="closeCredentialModal(false)"></div>
      <div ref="credPanel" class="relative z-10 w-full max-w-lg overflow-hidden rounded-xl border border-border/70 bg-surface shadow-2xl">
        <div class="flex items-center justify-between border-b border-border/60 px-5 py-4">
          <div>
            <h3 class="text-base font-semibold text-foreground">Update credential</h3>
            <p class="text-xs text-subtle-foreground">Enter a new API key. It won’t be shown again.</p>
          </div>
          <button
            ref="credCloseBtn"
            type="button"
        ...
        <div class="px-5 py-4">
          <label for="sp-api-key" class="text-xs font-semibold uppercase tracking-wide text-subtle-foreground">API key</label>
          <input
            id="sp-api-key"
            ref="credInput"
          ...
          />
          <p class="mt-2 text-xs text-subtle-foreground">Leave blank to keep the existing credential.</p>
        </div>
...
import KeyValueTableEditor, { type KeyValueRow } from '@/components/specialists/edit/KeyValueTableEditor.vue'
import JsonEditorModal from '@/components/specialists/edit/JsonEditorModal.vue'
import { upsertSpecialist, type Specialist, type SpecialistProviderDefaults } from '@/api/client'
...
const providerDropdownOptions = computed(() => props.providerOptions.map((opt) => ({ id: opt, label: opt, value: opt })))

const providerDefaultsForSelected = computed(() => {
  const prov = (draft.provider || '').trim()
  return (props.providerDefaults && prov && props.providerDefaults[prov]) || undefined
})

const defaultBaseURL = computed(() => providerDefaultsForSelected.value?.baseURL || '')

const headerTitle = computed(() => (baseline.value?.name ? 'Edit Specialist' : 'Create Specialist'))
const headerSubtitle = computed(() => (baseline.value?.name ? baseline.value.name : null))
...
const fieldErrors = computed<Record<string, string>>(() => {
  ...
  const computedBaseURL = draft.useDefaultEndpoint ? (defaultBaseURL.value || '') : draft.customBaseURL
  ...
  if (touched.value.has('system') && draft.system != null && typeof draft.system !== 'string') {
...
const errorsByTab = computed(() => {
  ...
  if (fieldErrors.value.extraHeaders) advanced.push(fieldErrors.value.extraHeaders)
  if (fieldErrors.value.extraParams) advanced.push(fieldErrors.value.extraParams)
...
const baselinePayload = computed(() => (baseline.value ? normalizePayload(baseline.value) : normalizePayload(buildPayloadFromDraft())))

const currentPayload = computed(() => normalizePayload(buildPayloadFromDraft()))

const isDirty = computed(() => stableStringify(baselinePayload.value) !== stableStringify(currentPayload.value))
...
function buildPayloadFromDraft(): Specialist {
  ...
  const baseURL = draft.useDefaultEndpoint ? (defaults?.baseURL || '') : draft.customBaseURL
...
async function onSave() {
  ...
  try {
    ...
    baseline.value = normalizePayload(saved)
    credentialDraft.value = ''
    nameLockedAfterSave.value = true
...
const promptDropdownOptions = computed(() => [
  { id: '', label: promptsLoading.value ? 'Loading…' : 'Select prompt', value: '' },
  ...availablePrompts.value.map((p) => ({ id: p.id, label: p.name, value: p.id })),
])

const versionDropdownOptions = computed(() => [
  { id: '', label: versionsLoading.value ? 'Loading…' : 'Select version', value: '' },
...
function applySelectedVersion() {
  ...
  if (isDirty.value && draft.system.trim()) {
    const ok = confirm('Apply this version and overwrite current prompt edits?')
    if (!ok) return
  }
...
function setToolAllowed(name: string, allowed: boolean) {
  ...
  allowTools.value = Array.from(next).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }))
}
...
const jsonModalTitle = computed(() => (jsonTarget.value === 'headers' ? 'Edit extra headers as JSON' : 'Edit extra params as JSON'))
const jsonModalSubtitle = computed(() => (jsonTarget.value === 'headers' ? 'Must be a JSON object of string values.' : 'Must be a JSON object.'))

const jsonModalText = computed(() => {
  const obj = jsonTarget.value === 'headers' ? extraHeadersObj.value : extraParamsObj.value
...
function initFromInitial(sp: Specialist) {
  ...
  if (!normalized.baseURL || (defaultUrl && normalized.baseURL === defaultUrl)) {
