    ...
    <section class="flex min-h-0 max-h-[55vh] flex-col overflow-hidden rounded-2xl border border-border/70 bg-surface p-4 space-y-3 lg:h-full lg:max-h-none">
      ...
      <form class="grid gap-3 md:grid-cols-2" @submit.prevent="handleCreateExperiment">
        ...
        <label class="text-sm">
          ...
          <input v-model="form.model" required placeholder="gpt-4o" class="w-full rounded border border-border/70 bg-surface-muted/60 px-3 py-2" />
        </label>
        <label class="text-sm">
          <span class="text-subtle-foreground mb-1">Slice (optional)</span>
          <input v-model="form.sliceExpr" placeholder="validation" class="w-full rounded border border-border/70 bg-surface-muted/60 px-3 py-2" />
        </label>
        <label class="text-sm md:col-span-2">
          <span class="text-subtle-foreground mb-1">Notes</span>
          <textarea v-model="form.notes" rows="2" class="w-full rounded border border-border/70 bg-surface-muted/60 px-3 py-2"></textarea>
        </label>
        <div class="md:col-span-2 flex gap-3 items-center">
          <button type="submit" class="rounded border border-border/70 px-3 py-2 text-sm font-semibold">Create experiment</button>
    ...
    <section class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-border/70 bg-surface p-4 gap-4 lg:h-full">
      <header class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-subtle-foreground">Launch runs and inspect outcomes.</p>
        </div>
        ...
        <div v-else class="space-y-3 min-w-0">
          ...
          <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            <div>
              ...
              <p class="text-xs text-subtle-foreground">Dataset: {{ experiment.datasetId }} · Variants: {{ experiment.variants.length }}</p>
            </div>
          ...
          <div class="text-sm">
            <button @click="toggleRuns(experiment.id)" class="text-accent text-sm hover:underline">
