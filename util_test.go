...
func TestConvertDockerResponse_Success(t *testing.T) {
	dresp := &DockerExecResponse{ReturnCode: 0, Stdout: "output", Stderr: ""}
	resp := convertDockerResponse(dresp)
...
func TestConvertDockerResponse_Error(t *testing.T) {
	dresp := &DockerExecResponse{ReturnCode: 1, Stdout: "", Stderr: "error occurred"}
	resp := convertDockerResponse(dresp)
