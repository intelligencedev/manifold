    ...
    <div class="p-4 h-full box-border">
      <div class="overflow-y-auto h-full pr-2 space-y-4">
        <div v-for="(nodes, category) in nodeCategories" :key="category">
          <div class="text-base font-bold mb-2 uppercase text-gray-300 cursor-pointer flex justify-between items-center p-2 bg-slate-700 border border-slate-600 rounded hover:bg-white/20" @click="toggleAccordion(category)">
        <span>{{ category }}</span>
        ...
        <div v-for="node in nodes" :key="node.type" class="p-3 mb-2 bg-slate-700 border border-slate-600 rounded cursor-grab text-base font-medium flex items-center justify-center hover:bg-white/20" draggable="true" @dragstart="(event) => onDragStart(event, node.type)">
          {{ node.type }}
        </div>
          </div>
        </div>
