...
func (s *pgChatStore) scanSession(row pgx.Row) (persistence.ChatSession, error) {
	...
	if err := row.Scan(&cs.ID, &cs.Name, &cs.Kind, &owner, &cs.CreatedAt, &cs.UpdatedAt, &cs.LastMessagePreview, &cs.Model, &cs.Summary, &cs.SummarizedCount, &cs.ProjectID); err != nil {
...
WITH ins AS (
  ...
  RETURNING id, name, kind, user_id, created_at, updated_at, last_message_preview, model, summary, summarized_count, project_id
)
SELECT id, name, kind, user_id, created_at, updated_at, last_message_preview, model, summary, summarized_count, project_id FROM ins
UNION ALL
SELECT id, name, kind, user_id, created_at, updated_at, last_message_preview, model, summary, summarized_count, project_id FROM chat_sessions WHERE id = $1
...
func (s *pgChatStore) ListSessionsByKind(ctx context.Context, userID *int64, kind string) ([]persistence.ChatSession, error) {
	kind = normalizeSessionKind(kind)
	query := `
SELECT id, name, kind, user_id, created_at, updated_at, last_message_preview, model, summary, summarized_count, project_id
FROM chat_sessions
WHERE kind = $1`
	args := []any{kind}
	if userID != nil {
...
func (s *pgChatStore) CreateSessionKind(ctx context.Context, userID *int64, name, kind string) (persistence.ChatSession, error) {
	...
	INSERT INTO chat_sessions (id, user_id, name, kind)
	VALUES ($1, $2, $3, $4)
	RETURNING id, name, kind, user_id, created_at, updated_at, last_message_preview, model, summary, summarized_count, project_id`, id, uid, name, kind)
