...
func (s *Service) Retrieve(ctx context.Context, q string, opt retrieve.RetrieveOptions) (retrieve.RetrieveResponse, error) {
    ...
    for i := 0; i < diag.FtCount; i++ { s.metrics.IncCounter("retrieval_candidates", map[string]string{"type": "fts", "tenant": plan.Tenant}) }
    for i := 0; i < diag.VecCount; i++ { s.metrics.IncCounter("retrieval_candidates", map[string]string{"type": "vec", "tenant": plan.Tenant}) }
    ...
    // Results counter
    for i := 0; i < len(items); i++ { s.metrics.IncCounter("retrieval_results_total", map[string]string{"tenant": plan.Tenant}) }
    ...
    if dm, ok := debug["diagnostics"].(map[string]any); ok {
        if gv, ok := addDbg["graph"]; ok {
            if gmap, ok := gv.(map[string]any); ok {
                if msVal, ok := gmap["ms"]; ok { dm["graph_ms"] = msVal }
            }
        }
