    ...
    >
      <div class="flex items-center justify-between">
        ...
        <RouterLink
          ...
          >Back to experiments</RouterLink
        >
      </div>
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr
                  ...
                  :class="{
                    'bg-accent/10': run.id === selectedRunId,
                    'hover:bg-muted/60': run.id !== selectedRunId,
                  }"
        ...
        >
          <header class="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p class="text-sm text-subtle-foreground">
                Run {{ selectedRun.id }} · Status {{ selectedRun.status }}
              </p>
            </div>
          ...
          <div class="grid gap-3 text-sm md:grid-cols-2">
            <div>
              ...
              <p>
                <span class="text-subtle-foreground">Completed:</span>
                {{ formatDate(selectedRun.endedAt) }}
              </p>
            </div>
            <div>
              <p>
                <span class="text-subtle-foreground">Error:</span>
                {{ selectedRun.error ?? "—" }}
              </p>
              <p>
                <span class="text-subtle-foreground">Plan shards:</span>
                {{ selectedRun.plan?.shards?.length ?? 0 }}
              </p>
            </div>
          </div>

          <div v-if="sortedMetrics.length" class="space-y-2">
            <h3 class="text-sm font-semibold text-subtle-foreground">
              Metrics
            </h3>
            <div class="flex flex-wrap gap-2">
              <span
                v-for="[name, value] in sortedMetrics"
                :key="name"
                class="rounded-full border border-border/70 px-3 py-1 text-xs"
              >
          ...
          <div class="flex min-h-0 flex-col">
            <h3 class="text-sm font-semibold text-subtle-foreground mb-2">
              Results
            </h3>
            <p
              v-if="loadingSelectedRunResults"
            ...
            <div
              v-else
              class="flex-1 min-h-0 overflow-auto overscroll-contain space-y-4 pr-1"
            >
              <article
              ...
              >
                ...
                >
                  <div class="font-medium">
                    Row {{ result.rowId }} · Variant {{ result.variantId }} ·
                  ...
                  <div>
                    <div class="flex items-center justify-between">
                      ...
                      <button
                        ...
                        @click="
                          openExpandedText('Rendered Prompt', result.rendered)
                        "
                      >
                        Expand
                      </button>
                    ...
                    <pre
                      class="mt-1 max-h-64 overflow-auto rounded bg-background/60 p-3 text-xs whitespace-pre-wrap"
                      >{{ result.rendered || "—" }}</pre
                    >
                  </div>
                  <div>
                ...
                <div class="grid gap-3 md:grid-cols-2">
                  <div>
                    ...
                    <pre
                      class="mt-1 max-h-48 overflow-auto rounded bg-background/60 p-3 text-xs whitespace-pre-wrap"
                      >{{ asPrettyJSON(result.expected) }}</pre
                    >
                  </div>
                  <div
                    v-if="result.scores && Object.keys(result.scores).length"
                  >
                    <h4
                      class="text-xs font-semibold text-subtle-foreground uppercase tracking-wide"
                    >
                ...
                <div
                  v-if="
                    result.artifacts && Object.keys(result.artifacts).length
                  "
                  class="text-xs"
                >
                  <h4
                    class="font-semibold text-subtle-foreground uppercase tracking-wide"
                  >
                    Artifacts
                  </h4>
...
const sortedMetrics = computed(() => {
  ...
  return Object.entries(selectedRun.value.metrics).sort(([a], [b]) =>
    a.localeCompare(b),
  );
});
...
function ensureRunSelection() {
  ...
  if (
    selectedRunId.value &&
    currentRuns.some((run) => run.id === selectedRunId.value)
  ) {
    return;
  }
