...
func NewExecutor(cfg config.ExecConfig, workdir string) *ExecutorImpl {
    ...
    for _, b := range cfg.BlockBinaries { blocked[b] = struct{}{} }
...
func (e *ExecutorImpl) Run(ctx context.Context, req ExecRequest) (ExecResult, error) {
    ...
    if err != nil {
        ...
        if errors.As(err, &ee) { exit = ee.ExitCode() } else if errors.Is(ctx.Err(), context.DeadlineExceeded) { exit = 124 } else { exit = 1 }
    }
    ...
    trunc := false
    if e.outLimit > 0 {
        if len(outS) > e.outLimit { outS = outS[:e.outLimit] + "\n[TRUNCATED]"; trunc = true }
        if len(errS) > e.outLimit { errS = errS[:e.outLimit] + "\n[TRUNCATED]"; trunc = true }
    }
