...
func (s *pgChatStore) scanSession(row pgx.Row) (persistence.ChatSession, error) {
	...
	if err := row.Scan(&cs.ID, &cs.Name, &cs.Kind, &owner, &cs.CreatedAt, &cs.UpdatedAt, &cs.LastMessagePreview, &cs.MessageCount, &cs.Model, &cs.Summary, &cs.SummarizedCount, &cs.ProjectID, &cs.MemoryEnabled, &cs.EvolvingMemoryEnabled, &cs.BeliefMemoryEnabled, &cs.ActiveSpecialist, &cs.ActiveTeam, &cs.Pinned); err != nil {
...
WITH ins AS (
  ...
  RETURNING id, name, kind, user_id, created_at, updated_at, last_message_preview, 0::int AS message_count, model, summary, summarized_count, project_id, memory_enabled, evolving_memory_enabled, belief_memory_enabled, active_specialist, active_team, pinned
)
SELECT id, name, kind, user_id, created_at, updated_at, last_message_preview, message_count, model, summary, summarized_count, project_id, memory_enabled, evolving_memory_enabled, belief_memory_enabled, active_specialist, active_team, pinned FROM ins
UNION ALL
...
func (s *pgChatStore) CreateSessionKind(ctx context.Context, userID *int64, name, kind string) (persistence.ChatSession, error) {
	...
	INSERT INTO chat_sessions (id, user_id, name, kind)
	VALUES ($1, $2, $3, $4)
	RETURNING id, name, kind, user_id, created_at, updated_at, last_message_preview, 0::int AS message_count, model, summary, summarized_count, project_id, memory_enabled, evolving_memory_enabled, belief_memory_enabled, active_specialist, active_team, pinned`, id, uid, name, kind)
...
func (s *pgChatStore) SetSessionActiveTarget(ctx context.Context, userID *int64, id string, activeSpecialist string, activeTeam string) (persistence.ChatSession, error) {
	activeSpecialist = strings.TrimSpace(activeSpecialist)
	activeTeam = strings.TrimSpace(activeTeam)
	query := `
UPDATE chat_sessions
SET active_specialist = $2, active_team = $3
WHERE id = $1`
	args := []any{id, activeSpecialist, activeTeam}
	if userID != nil {
		query += ` AND user_id = $4`
		args = append(args, *userID)
	}
