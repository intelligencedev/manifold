...
func markdownChunk(text string, opt ingest.ChunkingOptions) []Chunk {
	...
	idx := 0
	// flushUpToTarget emits chunks from buf ensuring no chunk exceeds tgt chars
	flushUpToTarget := func() {
		for buf.Len() > tgt {
			s := buf.String()
			cut := tgt
			// try to cut at last newline or space within window to keep boundaries tidy
			if i := strings.LastIndex(s[:cut], "\n"); i > tgt/2 {
				cut = i
			} else if i := strings.LastIndex(s[:cut], " "); i > tgt/2 {
				cut = i
			}
			...
			rest := strings.TrimLeft(s[cut:], "\n ")
			buf.Reset()
			buf.WriteString(rest)
		}
	}
	writeFlush := func() {
		// ensure we don't emit oversize chunks
		flushUpToTarget()
	...
	for i, ln := range lines {
		...
		buf.WriteString(ln)
		// Enforce hard cap: if we exceed target, split now
		if buf.Len() >= tgt {
			flushUpToTarget()
			// If boundary is nice (heading/paragraph), finalize current chunk
			if isHeading || isParaBreak {
				writeFlush()
			}
		}
	}
