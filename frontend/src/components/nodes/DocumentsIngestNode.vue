<template>
  <BaseNode :id="id" :data="data" :min-height="490" @resize="onResize">
    <template #header>
    ...
    <div class="mode-selector flex justify-around mb-2 text-sm">
      <label
        v-for="option in ['passthrough', 'documents', 'git']"
        :key="option"
    ...
    <BaseInput
      label="Chunk Overlap"
      type="number"
      v-model.number="chunk_overlap"
    />

    <BaseCheckbox label="Generate Summaries" v-model="generate_summary" />
    <BaseCheckbox label="Generate Keywords" v-model="generate_keywords" />
