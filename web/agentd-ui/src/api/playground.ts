...
export async function listPrompts(params?: {
  ...
  const { data } = await apiClient.get<{ prompts: Prompt[] }>(
    "/v1/playground/prompts",
    { params },
  );
  return data.prompts ?? [];
}
...
export async function createPrompt(payload: Partial<Prompt>): Promise<Prompt> {
  const { data } = await apiClient.post<Prompt>(
    "/v1/playground/prompts",
    payload,
  );
  return data;
...
export async function getPrompt(id: string): Promise<Prompt> {
  const { data } = await apiClient.get<Prompt>(
    `/v1/playground/prompts/${encodeURIComponent(id)}`,
  );
  return data;
}
...
export async function listPromptVersions(id: string): Promise<PromptVersion[]> {
  const { data } = await apiClient.get<{ versions: PromptVersion[] }>(
    `/v1/playground/prompts/${encodeURIComponent(id)}/versions`,
  );
...
export async function listDatasets(): Promise<Dataset[]> {
  const { data } = await apiClient.get<{ datasets: Dataset[] }>(
    "/v1/playground/datasets",
  );
...
export async function getDataset(id: string): Promise<Dataset> {
  const { data } = await apiClient.get<Dataset>(
    `/v1/playground/datasets/${encodeURIComponent(id)}`,
  );
  return data;
}
...
export async function updateDataset(
  ...
  const { data } = await apiClient.put<Dataset>(
    `/v1/playground/datasets/${encodeURIComponent(id)}`,
    payload,
  );
  return data;
...
export async function listExperiments(): Promise<ExperimentSpec[]> {
  const { data } = await apiClient.get<{ experiments: ExperimentSpec[] }>(
    "/v1/playground/experiments",
  );
  return data.experiments ?? [];
}

export async function getExperiment(id: string): Promise<ExperimentSpec> {
  const { data } = await apiClient.get<ExperimentSpec>(
    `/v1/playground/experiments/${encodeURIComponent(id)}`,
  );
  return data;
}
...
export async function createExperiment(
  ...
  const { data } = await apiClient.post<ExperimentSpec>(
    "/v1/playground/experiments",
    spec,
  );
  return data;
...
export async function startExperimentRun(experimentId: string): Promise<Run> {
  const { data } = await apiClient.post<Run>(
    `/v1/playground/experiments/${encodeURIComponent(experimentId)}/runs`,
    {},
  );
  return data;
}
...
export async function listRunResults(runId: string): Promise<RunResult[]> {
  const { data } = await apiClient.get<{ results: RunResult[] }>(
    `/v1/playground/runs/${encodeURIComponent(runId)}/results`,
  );
  return data.results ?? [];
}
