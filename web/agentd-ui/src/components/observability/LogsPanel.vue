  ...
  <div class="rounded-2xl border border-border/70 bg-surface p-6 shadow-lg flex h-full flex-col overflow-hidden">
    <div class="flex flex-wrap items-start justify-between gap-4 mb-4">
      <div>
        ...
        <p class="text-xs text-faint-foreground">Real-time application logs (Admin only)</p>
      </div>
    ...
    <div class="flex-1 overflow-auto rounded-lg border border-border/60 bg-surface-muted/40 p-3 font-mono text-xs">
      ...
      <div v-else class="space-y-1">
        ...
        >
          <span class="text-faint-foreground shrink-0">{{ formatTimestamp(log.timestamp) }}</span>
          <span class="font-semibold shrink-0 uppercase" :class="getLevelColorClass(log.level)">
            {{ log.level }}
          </span>
    ...
    <div class="mt-3 flex items-center justify-between text-xs text-faint-foreground">
