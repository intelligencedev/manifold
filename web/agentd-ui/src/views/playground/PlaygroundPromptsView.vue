    ...
    >
      <header class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-subtle-foreground">
            Manage existing prompts and versions.
          </p>
        </div>
        <input
          v-model="search"
          placeholder="Filter by name or tag"
          class="rounded border border-border/70 bg-surface-muted/60 px-3 py-2 text-sm w-64"
        />
      </header>
      ...
      <div class="flex-1 min-h-0 overflow-y-auto overscroll-contain pr-1">
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            >
              <td class="py-2 font-medium">
                <RouterLink
                  ...
                  >{{ prompt.name }}</RouterLink
                >
              </td>
              <td class="py-2 max-w-[280px] truncate">
                {{ prompt.description || "—" }}
              </td>
              <td class="py-2">{{ prompt.tags?.join(", ") || "—" }}</td>
              <td class="py-2 text-subtle-foreground">
            ...
            <tr v-if="store.promptsLoading">
              <td colspan="5" class="py-3 text-center text-subtle-foreground">
                Loading…
              </td>
            </tr>
      ...
      <div
        ...
        class="rounded border border-danger/60 bg-danger/10 px-3 py-2 text-danger-foreground text-sm"
      >
        {{ store.promptsError }}
      </div>
    </section>
