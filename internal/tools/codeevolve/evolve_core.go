...
// It accepts the full code and the evolvable sections separately.
...
var diffRegexp = regexp.MustCompile(`(?s)<<<<<<< SEARCH\n(.*?)\n=======\n(.*?)\n>>>>>>> REPLACE`)
...
func ApplyDiffsToCode(code string, diffs []DiffBlock) (string, error) {
	...
	for _, d := range diffs {
		if strings.Contains(updated, d.Search) {
			...
			// Skip this diff if the search text is not found.
			continue
		}
	}
	return updated, nil
...
func RunAlphaEvolve(
	...
	evalFunc func(string, []string) (map[string]float64, error),
	llmClient LLMClient,
	db ProgramDatabase,
	generations int,
	...
	scores, err := EvaluateProgram(prog.Code, prog.EvolvableSections, evalFunc)
	if err != nil {
