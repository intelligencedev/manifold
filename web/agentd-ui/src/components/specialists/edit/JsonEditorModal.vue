<template>
  ...
  >
    ...
    <div
      ref="panel"
      class="relative z-10 flex w-full max-w-4xl flex-col overflow-hidden rounded-xl border border-border/70 bg-surface shadow-2xl"
    >
      <div
      ...
      >
        <div>
          ...
          <p v-if="subtitle" class="text-xs text-subtle-foreground">
            {{ subtitle }}
          </p>
        </div>
        <button
      ...
      <div class="flex min-h-0 flex-1 flex-col gap-3 px-5 py-4">
        <div
          v-if="parseError"
          class="rounded border border-danger/60 bg-danger/10 p-3 text-sm text-danger-foreground"
        >
          {{ parseError }}
        </div>
