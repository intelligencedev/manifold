<template>
  <GlassCard :interactive="interactive" class="h-full">
    <div class="flex items-start justify-between gap-3">
      <div class="space-y-1">
        <p class="text-[11px] font-semibold uppercase tracking-wide text-subtle-foreground">{{ label }}</p>
        <p class="text-3xl font-semibold leading-tight text-foreground tabular-nums">{{ formattedValue }}</p>
        <p v-if="secondary" class="text-xs text-faint-foreground">{{ secondary }}</p>
      </div>
    ...
    <div v-if="deltaText" :class="deltaClasses" class="mt-4 inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold">
...
const deltaClasses = computed(() => {
  ...
  if (resolvedTrend.value === 'up') return [...base, 'border-success/40 text-success']
  if (resolvedTrend.value === 'down') return [...base, 'border-danger/40 text-danger']
