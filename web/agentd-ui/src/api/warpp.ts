...
export async function fetchWarppWorkflow(
  ...
  const resp = await fetch(
    `${apiBase}/workflows/${encodeURIComponent(intent)}`,
  );
  return handleResponse<WarppWorkflow>(resp);
}
...
export async function saveWarppWorkflow(
  ...
  const resp = await fetch(
    `${apiBase}/workflows/${encodeURIComponent(workflow.intent)}`,
    {
      method: "PUT",
  ...
  const resp = await fetch(`${apiBase}/run`, {
    ...
    body: JSON.stringify(
      projectId && projectId.trim()
        ? { intent, prompt, project_id: projectId.trim() }
        : { intent, prompt },
    ),
    signal,
  });
...
function deserializeTrace(entry: any): WarppStepTrace {
  ...
  const status =
    typeof entry.status === "string"
      ? (entry.status as WarppStepTrace["status"])
      : undefined;
  const mapped: WarppStepTrace = {
    stepId,
    text: typeof entry.text === "string" ? entry.text : undefined,
    renderedArgs: isPlainObject(entry.rendered_args)
      ? { ...entry.rendered_args }
      : undefined,
