...
- Save: opens a metadata modal for Description and Keywords (both required). The editor persists steps, edges (as depends_on), and UI metadata (layout, parents, groups, notes). Client caches UI to preserve layout even if the server doesn’t persist all UI fields.
...
- While running: the header shows a spinner and a live timer. The run view streams step traces (with small delays for readability) and logs messages.
- Result modal: click a step with trace to open a modal showing Rendered Arguments, Delta, Payload, and any error.
