...
func (o *OtelMetrics) getCounter(name string) (metric.Int64Counter, bool) {
    ...
    o.mu.Lock()
    defer o.mu.Unlock()
    if c, ok = o.counters[name]; ok { return c, true }
    ctr, err := o.meter.Int64Counter(name)
    if err != nil { return ctr, false }
    o.counters[name] = ctr
    return ctr, true
}
...
func (o *OtelMetrics) getHistogram(name string) (metric.Float64Histogram, bool) {
    ...
    o.mu.Lock()
    defer o.mu.Unlock()
    if h, ok = o.histograms[name]; ok { return h, true }
    hist, err := o.meter.Float64Histogram(name)
    if err != nil { return hist, false }
...
func toAttrs(labels map[string]string) []attribute.KeyValue {
    ...
    for k, v := range labels { out = append(out, attribute.String(k, v)) }
    return out
}
