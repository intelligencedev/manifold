...
func (s *PlaygroundStore) CreateExperiment(ctx context.Context, spec experiment.ExperimentSpec) (experiment.ExperimentSpec, error) {
	...
	_, err = s.exec(ctx, `INSERT INTO playground_experiments (id, user_id, payload) VALUES ($1,$2,$3)
...
func (s *PlaygroundStore) GetExperiment(ctx context.Context, id string) (experiment.ExperimentSpec, bool, error) {
	uid := userIDFromContext(ctx)
	payload, ok, err := s.queryOnePayload(ctx, `SELECT payload FROM playground_experiments WHERE id=$1 AND user_id=$2`, id, uid)
	if err != nil || !ok {
		return experiment.ExperimentSpec{}, ok, err
	}
...
func (s *PlaygroundStore) ListExperiments(ctx context.Context) ([]experiment.ExperimentSpec, error) {
	uid := userIDFromContext(ctx)
	payloads, err := s.queryPayloads(ctx, `SELECT payload FROM playground_experiments WHERE user_id=$1`, uid)
	if err != nil {
		return nil, err
	}
...
func (s *PlaygroundStore) CreateRun(ctx context.Context, run playground.Run) (playground.Run, error) {
	...
	uid := userIDFromContext(ctx)
	_, err = s.exec(ctx, `INSERT INTO playground_runs (id, experiment_id, user_id, payload) VALUES ($1,$2,$3,$4)
...
func (s *PlaygroundStore) UpdateRunStatus(ctx context.Context, id string, status playground.RunStatus, endedAt time.Time, errMsg string, metrics map[string]float64) error {
	...
	_, err = s.exec(ctx, `UPDATE playground_runs SET payload=$1 WHERE id=$2 AND user_id=$3`, payload, id, userIDFromContext(ctx))
	return err
}
...
func (s *PlaygroundStore) AppendResults(ctx context.Context, runID string, results []playground.RunResult) error {
	if s.db != nil {
		return s.appendSQLiteResults(ctx, runID, results)
	}
	batch := &pgx.Batch{}
	uid := userIDFromContext(ctx)
...
func (s *PlaygroundStore) ListRuns(ctx context.Context, experimentID string) ([]playground.Run, error) {
	uid := userIDFromContext(ctx)
	payloads, err := s.queryPayloads(ctx, `SELECT payload FROM playground_runs WHERE experiment_id=$1 AND user_id=$2`, experimentID, uid)
	if err != nil {
		return nil, err
	}
...
func (s *PlaygroundStore) ListRunResults(ctx context.Context, runID string) ([]playground.RunResult, error) {
	uid := userIDFromContext(ctx)
	payloads, err := s.queryPayloads(ctx, `SELECT payload FROM playground_run_results WHERE run_id=$1 AND user_id=$2`, runID, uid)
	if err != nil {
		return nil, err
	}
...
func (s *PlaygroundStore) appendSQLiteResults(ctx context.Context, runID string, results []playground.RunResult) error {
	...
	for _, res := range results {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO playground_run_results (id, run_id, user_id, payload) VALUES (?,?,?,?)
			ON CONFLICT (id) DO UPDATE SET run_id=excluded.run_id, payload=excluded.payload`, res.ID, runID, uid, payload); err != nil {
			return err
		}
	}
	return tx.Commit()
