    ...
    >
      <aside class="lg:w-72">
        ...
        >
          ...
          <template
            v-if="selectedCount === 1 && selectedNode && !paletteOverride"
          >
            ...
            <div class="text-[11px] text-subtle-foreground mb-3">
              <div class="font-mono flex items-center gap-1">
                ...
                <span class="truncate" :title="selectedNode.id">{{
                  selectedNode.id
                }}</span>
              </div>
            </div>
          ...
          <template v-else>
            ...
            <p class="mt-1 text-xs text-subtle-foreground">
              Drag onto the canvas to add workflow steps, utilities, or group
              containers.
            </p>
            ...
            >
              <div class="space-y-2">
                <h3
                  class="text-[11px] font-semibold uppercase tracking-wide text-faint-foreground"
                >
                  Utility Nodes
                </h3>
                ...
                <div
                  ...
                  @dragstart="
                    (event: DragEvent) => onPaletteDragStart(event, tool)
                  "
                  @dragend="onPaletteDragEnd"
                >
          ...
          >
            ...
            <Panel position="bottom-left">
              <div class="ap-chip flex items-center gap-1 rounded-md p-1">
                ...
                <span
                  class="mx-0.5 h-5 w-px bg-border/60"
                  aria-hidden="true"
                ></span>
                <button
                  type="button"
      ...
      >
        ...
        >
          <div class="space-y-1">
            ...
            <p v-if="modalStepId" class="text-xs text-subtle-foreground">
              ID: {{ modalStepId }}
            </p>
            <p v-if="modalStatusLabel" class="text-xs text-subtle-foreground">
          ...
          <div v-if="activeModalTrace" class="space-y-5">
            <section v-if="activeModalTrace?.renderedArgs">
              ...
              >
                ...
                <svg
                  class="h-3.5 w-3.5 text-subtle-foreground transition-transform"
                  :class="collapsedArgs ? '-rotate-90' : 'rotate-0'"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
            ...
            <section v-if="activeModalTrace?.delta">
              ...
              >
                ...
                <svg
                  class="h-3.5 w-3.5 text-subtle-foreground transition-transform"
                  :class="collapsedDelta ? '-rotate-90' : 'rotate-0'"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
            ...
            <section v-if="activeModalTrace?.payload">
              ...
              >
                ...
                <svg
                  class="h-3.5 w-3.5 text-subtle-foreground transition-transform"
                  :class="collapsedPayload ? '-rotate-90' : 'rotate-0'"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
            ...
            <p
              v-if="activeModalTrace?.error"
              class="rounded border border-danger/40 bg-danger/10 px-3 py-2 text-xs text-danger-foreground"
            >
    ...
    >
      ...
      >
        ...
        <div class="px-5 py-4 space-y-4">
          <div>
            ...
            <p class="mt-1 text-[10px] text-faint-foreground">
              Provide a concise, multi-line description.
            </p>
          </div>
          <div>
            <label
            ...
            />
            <p class="mt-1 text-[10px] text-faint-foreground">
              Enter a comma-separated list. Both description and keywords are
              required.
            </p>
          </div>
        </div>
        ...
        >
          <button
            type="button"
            class="rounded px-3 py-1 text-sm font-medium bg-muted text-foreground hover:bg-muted/80"
            @click="closeMetaModal"
          >
            Cancel
          </button>
          <button
            type="button"
            class="rounded px-3 py-1 text-sm font-medium bg-accent text-accent-foreground hover:bg-accent/90 disabled:opacity-50 disabled:cursor-not-allowed"
...
function buildNodeStyle(kind: NodeKind, stored?: LayoutEntry) {
  ...
  const storedHeight =
    typeof stored?.height === "number" ? stored.height : undefined;
  const width =
    storedWidth && storedWidth >= defaults.minWidth
      ? storedWidth
      : defaults.defaultWidth;
...
const selectedNode = computed(() =>
  selectedCount.value === 1 ? selectedNodes.value[0] : null,
);
...
const workflowTools = computed(() =>
  tools.value.filter((tool) => !isUtilityToolName(tool.name)),
);
const utilityTools = computed(() =>
  tools.value.filter((tool) => isUtilityToolName(tool.name)),
);
const paletteSearch = ref("");
const paletteSearchTerm = computed(() =>
  paletteSearch.value.trim().toLowerCase(),
);
...
const filteredWorkflowTools = computed(() => {
  ...
  return workflowTools.value.filter(
    (tool) =>
      matchesSearch(tool.name, term) ||
      (tool.description ? matchesSearch(tool.description, term) : false),
  );
});
...
const modalStepTitle = computed(() => {
  ...
  return (
    resultModal.value.title ||
    activeModalTrace.value?.text ||
    resultModal.value.stepId
  );
});
...
const canSave = computed(
  () => !!activeWorkflow.value && !saving.value && dirty.value,
);
...
const canDelete = computed(
  () => !!activeWorkflow.value && !saving.value && !running.value,
);
...
const currentEdgeStyle = computed<EdgeStyle>(
  () => EDGE_STYLES[edgeStyleIndex.value],
);
const edgeStyleLabel = computed(
  () => EDGE_STYLE_LABELS[currentEdgeStyle.value],
);
const edgeStyleButtonTitle = computed(
  () => `Edge style: ${edgeStyleLabel.value}. Click to cycle`,
);
const edgeStyleAriaLabel = computed(
...
function onAutoLayout(direction: DagreDirection) {
  ...
  for (const n of topLevelNodes) {
    ...
    g.setNode(n.id, {
      width: size.width || DAGRE_NODE_BASE_WIDTH,
      height: size.height || DAGRE_NODE_BASE_HEIGHT,
    });
  }
  ...
  for (const e of edges.value) {
    ...
    const sourceGroupId = isStepLikeNode(sourceNode)
      ? (sourceNode.data as StepNodeData).groupId
      : undefined;
    const targetGroupId = isStepLikeNode(targetNode)
      ? (targetNode.data as StepNodeData).groupId
      : undefined;
    ...
    if (isStepLikeNode(n)) {
      ...
      if (data.groupId) {
        ...
        if (delta) {
          ...
          return {
            ...n,
            position: { x: oldPos.x + delta.dx, y: oldPos.y + delta.dy },
          };
        }
...
const metaSaveDisabled = computed(
  () =>
    ...
    parseKeywords(metaKeywords.value).length === 0,
);
...
function nextStepOrder(): number {
  return nodes.value.filter(
    (node) => isStepLikeNode(node) && Boolean((node.data as any)?.step),
  ).length;
}

function translateGroupChildren(
  groupId: string,
  dx: number,
  dy: number,
): boolean {
...
const latestUiSnapshot = ref<UiSnapshot>({
  layout: {},
  parents: {},
  groups: [],
  memberships: {},
...
async function performSave(
  ...
  try {
    ...
    console.log(
      ...
      Object.keys(normalizedSaved.ui?.layout ?? {}),
    );
...
async function onDelete() {
  ...
  const confirmed = window.confirm(
    `Delete workflow "${intent}"? This cannot be undone.`,
  );
  if (!confirmed) return;
  try {
...
async function onNew() {
  ...
  if (
    workflowList.value.some((w) => w.intent === intent) ||
    localWorkflows.value.has(intent)
  ) {
...
async function onImportSelected(event: Event) {
  ...
  let intent: string = normalizeIntent(
    String(data?.intent || data?.name || ""),
  );
  if (!intent) {
    const provided = window.prompt(
      "Enter an intent for this imported workflow:",
  ...
  const exists = (i: string) =>
    workflowList.value.some((w) => w.intent === i) ||
    localWorkflows.value.has(i);
  if (exists(intent)) {
    ...
    const rename = window.prompt(
      "A workflow with this intent already exists. Enter a new name:",
      candidate,
    );
  ...
  const wf: WarppWorkflow = {
    ...
    steps: steps.map((s: any) => ({
      ...
      publish_result:
        typeof s?.publish_result === "boolean" ? s.publish_result : undefined,
      publish_mode:
        s?.publish_mode === "immediate" || s?.publish_mode === "topo"
          ? s.publish_mode
          : undefined,
      continue_on_error:
        typeof s?.continue_on_error === "boolean"
          ? s.continue_on_error
          : undefined,
      tool:
        s?.tool && typeof s.tool?.name === "string"
          ? { name: s.tool.name, args: s.tool.args }
          : undefined,
      depends_on: Array.isArray(s?.depends_on)
        ? s.depends_on.filter((x: any) => typeof x === "string")
        : undefined,
    })),
    ui:
      data?.ui && typeof data.ui === "object"
        ? {
            layout:
              data.ui.layout &&
              typeof data.ui.layout === "object" &&
              !Array.isArray(data.ui.layout)
                ? { ...data.ui.layout }
                : undefined,
            parents:
              data.ui.parents &&
              typeof data.ui.parents === "object" &&
              !Array.isArray(data.ui.parents)
                ? { ...data.ui.parents }
                : undefined,
