<template>
  <div class="overflow-hidden rounded-2xl border border-border/70 bg-surface shadow-lg">
    <table class="min-w-full divide-y divide-border/60">
      ...
      <tbody class="divide-y divide-border/60">
        <tr v-for="run in runs" :key="run.id" class="hover:bg-surface-muted/60">
          <td class="whitespace-nowrap px-4 py-3 text-sm font-mono text-muted-foreground">
            {{ run.id }}
          </td>
          <td class="max-w-xl px-4 py-3 text-sm text-muted-foreground">
            <span class="line-clamp-2">{{ run.prompt }}</span>
          </td>
          <td class="whitespace-nowrap px-4 py-3 text-sm text-subtle-foreground">
            {{ run.tokens ?? '—' }}
          </td>
          <td class="whitespace-nowrap px-4 py-3 text-sm text-subtle-foreground">
        ...
        <tr v-if="!runs.length">
          <td colspan="5" class="px-4 py-8 text-center text-sm text-subtle-foreground">
            No runs yet.
          </td>
        </tr>
      </tbody>
