    ...
    <div class="mt-4 min-h-0 flex-1 overflow-hidden">
      ...
      <div v-else class="flex h-full flex-col gap-4 overflow-hidden">
        <div class="grid shrink-0 grid-cols-2 gap-3 xl:grid-cols-4">
          ...
          >
            <p
              class="text-[10px] font-semibold uppercase tracking-wide text-faint-foreground"
            >
              {{ metric.label }}
            </p>
            <p
              class="mt-1 text-xl font-semibold leading-none text-foreground tabular-nums"
            >
              {{ metric.value }}
            </p>
        ...
        <div class="grid min-h-0 flex-1 gap-4 overflow-hidden xl:grid-cols-2">
          ...
          <div
            class="flex min-h-0 flex-col overflow-hidden rounded-lg border border-border/60 bg-muted/10 p-4"
          >
            ...
            <div
              v-if="!sizeRows.length"
              class="mt-4 text-sm text-faint-foreground"
            >
            ...
            >
              <div v-for="row in sizeRows" :key="`${row.user}:${row.session}`">
                <div class="flex items-center justify-between gap-3 text-xs">
                  ...
                  <span class="shrink-0 tabular-nums text-faint-foreground">{{
        ...
        <p
          ...
          class="shrink-0 truncate text-[11px] text-faint-foreground"
        >
          {{ data.warnings[0] }}
        </p>
      </div>
      ...
      h(
        ...
        [
          h(
            ...
            {
              class:
                "font-mono text-[11px] uppercase tracking-[0.12em] text-faint-foreground",
            },
            props.title,
          ),
          props.rows.length === 0
                    ...
                    h(
                      ...
                      [
                        ...
                        h(
                          ...
                          {
                            class:
                              "shrink-0 tabular-nums text-faint-foreground",
                          },
